document.getElementById("predictForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (resultData.prediction[0] == 1 || resultData.probability[1] > 0.5) {
      result.innerHTML = `Diabetes Prediction: ${risk} Risk!<br>Please consult a doctor.<br>You are ${Math.round(
        resultData.probability[1] * 100
      )}% likely to have diabetes.`;
    } else {
      result.innerHTML = `Diabetes Prediction: ${risk} Risk!<br>You are ${Math.round(
        resultData.probability[1] * 100
      )}% likely to have diabetes`;
    }
